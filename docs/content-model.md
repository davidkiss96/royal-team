...
| `priceType` | enum: `fixed` \| `from` | Yes | Structured distinction between a flat price and a "starting from" price, rather than encoding it into free text — see the rationale below. |
| `amount` | number | Yes | Validated as a positive integer (`Rule.integer().positive()`) — whole forint amounts, no decimal places, matching standard Hungarian pricing convention. |
...
- **No formatted price string field** (e.g., editors typing "35 000 Ft-tól" directly) — the display string is composed entirely in the Next.js presentation layer from `priceType` + `amount`. This is a direct, concrete application of the project's core boundary ("Sanity provides content, Next.js provides presentation"): letting editors hand-type formatted price strings would risk inconsistent thousands-separators, spacing, or "Ft"/"-tól" phrasing across dozens of line items, whereas deriving the display format once in code guarantees every price renders identically.
