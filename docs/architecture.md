...
- `npm run cf:preview` — builds with `NEXT_PUBLIC_SITE_ENV=preview` and `NEXT_PUBLIC_SITE_URL=https://preview.royalteamszerviz.hu` (prefixed directly on the build command, same pattern as `cf:deploy` — Section 17.2/17.3), then runs `wrangler versions upload --preview-alias staging`: uploads a new Worker **version** to the real `royal-team` Worker and attaches the stable `staging` preview alias, without touching which version serves production traffic (`wrangler deploy`/`wrangler versions deploy` are the only commands that do that). Resulting URL is stable across repeated preview uploads: `https://staging-royal-team.royal-team.workers.dev`. Every preview upload is `noindex, nofollow` with `robots.txt: Disallow: /`, regardless of content. Shares the same `RESEND_API_KEY`/`RESEND_FROM_EMAIL` Secrets as production (Secrets are Worker-wide, not per-version) — the contact form genuinely works on the preview.
- `npm run cf:preview:local` — builds and runs the actual Cloudflare Worker locally (via `wrangler`/`workerd`, not `next start`, and not uploaded anywhere) — useful for validating a change under the Cloudflare runtime before even using up a preview upload. Non-production, no `NEXT_PUBLIC_SITE_URL` set (falls back to `localhost:3000`).
- `npm run cf:deploy` — the one script that builds as production (`NEXT_PUBLIC_SITE_ENV=production`, prefixed directly in the script — see Section 17.2) and runs `wrangler deploy`, which both uploads a version and immediately routes 100% of production traffic to it. Requires `wrangler login` (or a Cloudflare API token) locally.
...
- `public/.assetsignore` — gitignore-syntax file Wrangler reads from the assets directory root (`.open-next/assets/.assetsignore` — OpenNext copies `public/`'s contents there verbatim, dotfiles included, which is how it gets there) to exclude files from the actual upload. Currently just `.DS_Store`: macOS silently recreates this file in any folder Finder browses, and since it's untracked by git, `git status` never surfaces it — but every local `cf:build`/`cf:preview`/`cf:deploy` was still bundling and publicly serving whatever copy happened to exist in `public/` at build time (verified: it was live on both the production and preview Workers before this fix). `.gitignore` alone can't fix this — it only affects git, not what OpenNext copies into the deploy artifact.
...
| `NEXT_PUBLIC_SITE_ENV` | build-time | `cf:deploy` (`production`) or `cf:preview` (`preview`) — never `.env.local`, never the Dashboard |
| `NEXT_PUBLIC_SITE_URL` | build-time | `cf:preview` only (`https://preview.royalteamszerviz.hu`) — ignored for production, which always uses the hardcoded `PRODUCTION_SITE_URL` constant instead |
...
**Manual, one-time Cloudflare dashboard steps not covered by any file in this repo:** connecting the Worker to a Cloudflare account/project, setting the two Resend Secrets (already done), attaching the `royalteamszerviz.hu` custom domain, and the `preview.royalteamszerviz.hu` redirect setup (Section 17.3).
...
- **Preview** (`npm run cf:preview`): sets `NEXT_PUBLIC_SITE_ENV=preview` and `NEXT_PUBLIC_SITE_URL=https://preview.royalteamszerviz.hu` directly on its build command (see 17.3 for why that specific domain). `robots.txt` disallows everything; every page's `robots` meta is `noindex, nofollow` regardless of content; canonical/OG/Twitter URLs resolve against that preview domain — never the production one, and never claiming to be it.
- **Everything else** (`next dev`, plain `next build`/`cf:build`, `cf:preview:local`): same non-indexable behavior as preview, but with no `NEXT_PUBLIC_SITE_URL` set, so canonical/OG/Twitter fall back to `http://localhost:3000`. This is the default with no configuration required, so nothing can become accidentally indexable by omission.
...
### 17.3 Preview custom domain (`preview.royalteamszerviz.hu`) — platform limitation, chosen workaround

The goal was for `staging`'s preview alias (`https://staging-royal-team.royal-team.workers.dev`) to also be reachable at `https://preview.royalteamszerviz.hu`, so the business owner never sees a `workers.dev` URL. **Verified directly against Cloudflare's own docs: this isn't currently possible as a direct binding.** Preview URLs/aliases (`wrangler versions upload --preview-alias`) are documented as workers.dev-only ("You cannot currently configure Preview URLs to run on a subdomain other than workers.dev") — Cloudflare Custom Domains and Routes only ever bind a hostname to a Worker's currently-*promoted* (production) version, never to a specific non-promoted version or alias. There is no config in `wrangler.jsonc` that changes this.

**Chosen workaround: a Cloudflare Redirect Rule, not a second Worker.** Rather than standing up a dedicated proxy Worker (extra infra, extra maintenance, and still not a *transparent* solution), `preview.royalteamszerviz.hu` should 301/302-redirect to the current `staging-royal-team.royal-team.workers.dev` URL. This is a standard, well-documented Cloudflare pattern requiring **zero application code, zero new Workers, and zero `wrangler.jsonc` changes** — it's pure zone-level (Dashboard) configuration, so it cannot be completed from this repository. Manual, one-time setup (Cloudflare Dashboard, on the `royalteamszerviz.hu` zone):

1. DNS → add a **proxied** (orange-cloud) `A` record: `preview` → `192.0.2.1` (a reserved, non-routable placeholder address — standard practice for a hostname that exists only to be caught by a redirect/rule, never to reach a real origin).
2. Rules → Redirect Rules → new rule: when hostname equals `preview.royalteamszerviz.hu`, redirect to `https://staging-royal-team.royal-team.workers.dev${uri.path}` (dynamic, path-preserving), status 302 (302, not 301, since the alias's underlying content will change on every future preview upload — a 301 risks being over-cached by browsers/intermediaries pointing at a stale target).

Once set up, this needs no further maintenance: `staging-royal-team.royal-team.workers.dev` always reflects whatever the most recent `npm run cf:preview` uploaded, and the redirect target string never has to change. **Not yet configured** — the site-config.ts `SITE_URL`/canonical logic (17.2) is already correct and forward-compatible with this domain regardless of whether the redirect exists yet.
