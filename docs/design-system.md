...
- The Contact page's map was left in its static-placeholder prototype form pending Next.js implementation-phase work — **now implemented**: a real, interactive Google Maps embed, click-to-load (no iframe/no request to Google until the visitor clicks "Térkép betöltése"), with an external "Megnyitás Google Térképen" link once loaded (`docs/architecture.md` Section 9.1). The `/projektek/[slug]` per-project routing (currently a single hardcoded detail page, not parametrized) remains **confirmed to stay in its current prototype form** for now — real slug-based routing is still explicitly Next.js implementation-phase work, not something to resolve in design or content-model documents now.
...
| `dpf` (one hardcoded detail page) | Hero, problem section, numbered process, benefits + CTA card, dedicated FAQ | `/szolgaltatasok/[slug]` — **generalize this into the standard `Service` detail template**, not a one-off DPF-specific page. | Individual `Service` document. **Resolved during Next.js implementation:** hero/tagline/problem section map to existing fields (`heroImage`, `tagline`, `body`, `gallery`); benefits reuse the existing flat `highlights` field (no richer title/description shape added); FAQ renders `Service.faq`, its correct home now that FAQ display is per-service; the numbered process section needed one new field, `Service.process` (`docs/content-model.md` Section 0 item 8, Section 2). |
| `pricing` (**new in Revision 2**) | Hero, category sections (each a list of name/note/price rows), disclaimer note, CTA banner | `/arlista` | **Resolved.** `PriceCategory` documents (`docs/content-model.md` Section 10) — fully independent of `Service`, with embedded price items (name, note, `priceType`/`amount` pair, per-item `isActive`). **Resolved during Next.js implementation:** the reference's "Ingyenes" quote-only line items needed a third `priceType` value, `quote` (`docs/content-model.md` Section 0 item 10) — `amount` doesn't apply to those rows. |
...
| `project-amg` (detail, **new in Revision 2**) | Hero, overview + structured spec table (2-col), work-performed section, gallery, results section, related-services + CTA | `/projektek/[slug]` (see routing note above) | Individual `Project` document — `body` (overview/narrative), `specs` (the structured spec table — new field, `docs/content-model.md` Section 4), `gallery`, `relatedServices` (**plural**, corrected in this revision — the design shows three related services per project, not one). **Resolved during Next.js implementation:** the "work performed" numbered list and the customer pull-quote both render from `body` (its own description already covers "modifications → process → testing → results"); the "Eredmény" stat tiles needed one new field, `Project.results` (`docs/content-model.md` Section 0 item 9, Section 4). |
...
Unchanged in substance from the original review (Section 14) — tokenize properly, build the shared component set once, use real routing, trim the unused shadcn scaffold, drop the Figma-Make-specific image-fallback component, replace the RTL layout trick, verify accessibility gaps rather than assuming the design is compliant as-is. **Added from this revision:** implement real per-project `/projektek/[slug]` routing (still pending) — the real Google Maps click-to-load embed is now implemented (`docs/architecture.md` Section 9.1); wire in the real logo SVG in place of the procedural placeholder.
